    ...
    <div style={{ padding: '1rem' }}>
      ...
      {project && (
        <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: '0.9rem' }}>
          <div style={panel}>
            <h3 style={{ marginTop: 0, marginBottom: '0.6rem' }}>组件 到 集群 到 节点</h3>
                  ...
                  {isOpen && linkedClusters.map((cluster) => {
                    ...
                    return (
                      <div key={cluster.id} style={{ marginLeft: '1.4rem' }}>
                        <div style={treeRow} onClick={() => toggle(cluster.id)} title="点击展开或收起">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
            <div style={panel}>
              <h3 style={{ marginTop: 0, marginBottom: '0.6rem' }}>组件跨集群关系图</h3>
              <ComponentClusterMap
                project={project}
                focusedComponentId={focusedComponentId}
                onComponentClick={setFocusedComponentId}
                onClusterClick={(clusterId) => {
                  const cluster = project.clusters.find((c) => c.id === clusterId)
                  if (cluster && cluster.nodes[0]) onActivateNode(clusterId, cluster.nodes[0].id)
                }}
              />
            </div>

            <div style={panel}>
              <h3 style={{ marginTop: 0, marginBottom: '0.6rem' }}>组件到集群关联矩阵</h3>
                          ...
                          {project.clusters.map((cluster) => {
                            ...
                            return (
                              <td key={cluster.id} style={{ ...td, textAlign: 'center' }}>
                                <button onClick={() => onToggleLink(comp.id, cluster.id)} style={{ ...matrixBtn, background: linked ? 'var(--accent)' : 'var(--bg)', color: linked ? '#fff' : 'var(--text-muted)', borderColor: linked ? 'var(--accent)' : 'var(--border)' }}>
                                  {linked ? 'Y' : 'N'}
                                </button>
                              </td>
                            )
...
function ComponentClusterMap({
  ...
  const compGap = project.components.length <= 1 ? 0 : (width - 80 - compW) / (project.components.length - 1)
  const clusterGap = project.clusters.length <= 1 ? 0 : (width - 80 - clusterW) / (project.clusters.length - 1)
  const compPos = project.components.map((c, i) => ({ id: c.id, x: 40 + i * compGap, y: compY, name: c.name }))
  const clusterPos = project.clusters.map((c, i) => ({ id: c.id, x: 40 + i * clusterGap, y: clusterY, name: c.name }))
  const cMap = new Map(compPos.map((n) => [n.id, n]))
  const gMap = new Map(clusterPos.map((n) => [n.id, n]))

  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="220" role="img" aria-label="组件与集群关系图">
      {project.components.flatMap((comp) =>
        comp.linkedClusterIds.map((cid) => {
          const from = cMap.get(comp.id)
          const to = gMap.get(cid)
          if (!from || !to) return null
          const active = !focusedComponentId || focusedComponentId === comp.id
          return <line key={`${comp.id}-${cid}`} x1={from.x + compW / 2} y1={from.y + compH} x2={to.x + clusterW / 2} y2={to.y} stroke={active ? '#60a5fa' : '#64748b'} strokeOpacity={active ? 0.95 : 0.35} strokeWidth={active ? 1.5 : 1} />
        }),
      )}
      {compPos.map((n) => (
        <g key={n.id} onClick={() => onComponentClick(n.id)} style={{ cursor: 'pointer' }}>
          <rect x={n.x} y={n.y} width={compW} height={compH} rx={8} ry={8} fill={focusedComponentId === n.id ? 'rgba(59,130,246,0.28)' : 'rgba(59,130,246,0.14)'} stroke={focusedComponentId === n.id ? '#2563eb' : '#3b82f6'} strokeWidth={focusedComponentId === n.id ? 2 : 1} />
          <text x={n.x + 8} y={n.y + 21} fontSize="11" fill="var(--text)">{n.name}</text>
        </g>
      ))}
      {clusterPos.map((n) => (
        <g key={n.id} onClick={() => onClusterClick(n.id)} style={{ cursor: 'pointer' }}>
          <rect x={n.x} y={n.y} width={clusterW} height={clusterH} rx={8} ry={8} fill="rgba(16,185,129,0.14)" stroke="#10b981" />
          <text x={n.x + 8} y={n.y + 21} fontSize="11" fill="var(--text)">{n.name}</text>
        </g>
      ))}
    </svg>
...
function roleLabel(role: string): string {
  const map: Record<string, string> = { primary: '主节点', standby: '从节点', publisher: '发布者', subscriber: '订阅者' }
  return map[role] ?? role
}

function nodeDot(role: string): CSSProperties {
  const colorMap: Record<string, string> = { primary: '#4ade80', publisher: '#60a5fa', standby: '#94a3b8', subscriber: '#c4b5fd' }
  return { display: 'inline-block', width: '8px', height: '8px', borderRadius: '50%', background: colorMap[role] ?? '#94a3b8', flexShrink: 0 }
}

const panel: CSSProperties = { border: '1px solid var(--border)', borderRadius: '10px', background: 'var(--bg-secondary)', padding: '0.75rem' }
const treeRow: CSSProperties = { display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.3rem 0.4rem', borderRadius: '6px', cursor: 'default', userSelect: 'none', fontSize: '0.85rem' }
