...
type TreeState = Record<string, boolean> // id -> expanded
    ...
    <div style={{ padding: '1rem' }}>
      ...
      {project && (
        <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: '0.9rem' }}>
          ...
          {/* ── Left: TreeView Topology ─────────────────────────── */}
          <div style={panel}>
            <h3 style={{ marginTop: 0, marginBottom: '0.6rem' }}>组件 → 集群 → 节点</h3>
            {project.components.length === 0 && (
              <div style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>暂无组件，请先创建。</div>
            )}
            {project.components.map((comp) => {
              const isOpen = expanded[comp.id] !== false // default open
              const linkedClusters = project.clusters.filter((c) =>
                comp.linkedClusterIds.includes(c.id),
              )
              return (
                <div key={comp.id} style={{ marginBottom: '0.5rem' }}>
                  ...
                  <div
                    ...
                    title="点击展开/收起"
                  >
                    <span style={{ color: 'var(--accent)', fontSize: '0.7rem', width: '1rem' }}>
                      {isOpen ? '▼' : '▶'}
                    </span>
                  ...
                  {isOpen && linkedClusters.map((cluster) => {
                    const cOpen = expanded[cluster.id]
                    return (
                      <div key={cluster.id} style={{ marginLeft: '1.4rem' }}>
                        <div
                          ...
                          title="点击展开/收起"
                        >
                          <span style={{ color: 'var(--text-muted)', fontSize: '0.65rem', width: '1rem' }}>
                            {cOpen ? '▼' : '▶'}
                          </span>
                          <span style={icon}>⬡</span>
                        ...
                        {cOpen && cluster.nodes.map((node) => (
                          <div
                            key={node.id}
                            style={{ ...treeRow, marginLeft: '1.4rem', cursor: 'pointer' }}
                            onClick={() => onActivateNode(cluster.id, node.id)}
                        ...
                        {cOpen && linkedClusters.length > 0 && cluster.nodes.length === 0 && (
                          <div style={{ marginLeft: '2.8rem', fontSize: '0.78rem', color: 'var(--text-muted)', padding: '0.2rem 0' }}>
                            暂无节点
                          </div>
                        )}
                      </div>
                  ...
                  {isOpen && linkedClusters.length === 0 && (
                    <div style={{ marginLeft: '2.2rem', fontSize: '0.78rem', color: 'var(--text-muted)', padding: '0.2rem 0' }}>
                      未关联任何集群
                    </div>
                  )}
                </div>
          ...
          {/* ── Right: Link Matrix + Component Cards ─────────────── */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
            {/* Link matrix */}
            <div style={panel}>
              <h3 style={{ marginTop: 0, marginBottom: '0.6rem' }}>组件 ↔ 集群 关联矩阵</h3>
              {project.clusters.length === 0 && (
                <div style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>暂无集群。</div>
              )}
              {project.clusters.length > 0 && (
                            ...
                            return (
                              <td key={cluster.id} style={{ ...td, textAlign: 'center' }}>
                                <button
                                  ...
                                  style={{
                                    ...matrixBtn,
                                    background: linked ? 'var(--accent)' : 'var(--bg)',
            ...
            <div style={panel}>
              ...
              {project.components.length === 0 && (
                <div style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>暂无组件。</div>
              )}
              {project.components.map((comp) => {
                const linkedClusters = project.clusters.filter((c) =>
                  comp.linkedClusterIds.includes(c.id),
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '0.76rem', color: 'var(--text-muted)', marginTop: '0.15rem' }}>
                          类型：{comp.componentType} | 关联集群：{linkedClusters.length} 个
                        </div>
                      </div>
                          ...
                          {linkedClusters.map((c) => (
                            <span key={c.id} style={clusterChip}>
                              ...
                              <span
                                ...
                                style={{ cursor: 'pointer', marginLeft: '0.3rem', color: 'var(--red)' }}
                                title="取消关联"
                              >
                                ×
                              </span>
...
function nodeDot(role: string): CSSProperties {
  const colorMap: Record<string, string> = {
    primary: '#4ade80', publisher: '#60a5fa',
    standby: '#94a3b8', subscriber: '#c4b5fd',
  }
  return {
...
const smallBtnDanger: CSSProperties = {
  ...btn, padding: '0.25rem 0.6rem', fontSize: '0.78rem', color: 'var(--red)',
}
...
const clusterChip: CSSProperties = {
